...
SELECT DISTINCT CONCAT_WS(" ", t1.firstname,t1.surname) AS member_name,
       f.name AS facility_name
FROM (SELECT m.firstname,m.surname,b.facid
      FROM Members AS m
      INNER JOIN Bookings AS b
      ON m.memid=b.memid
      WHERE b.facid IN (0,1) AND m.memid!=0) AS t1
...
SELECT f.name as facility_name,
                 CASE WHEN b.memid!=0 THEN CONCAT_WS(" ",m.firstname,  
                       m.surname) 
                             ELSE 'GUEST' 
                             END AS member_name,        
